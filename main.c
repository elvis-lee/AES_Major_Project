...
const unsigned char invsbox[16][16] = {
	...
	{0x90, 0xd8, 0xab, 0x0, 0x8c, 0xbc, 0xd3, 0x0a, 0xf7, 0xe4, 0x58, 0x5, 0xb8, 0xb3, 0x45, 0x06},
...
void readEncryptFile (unsigned char* buffer_ptr)
{
    ...
    FILE *fp = fopen("buildings.aes", "rb");

    if (!fp)
        printf("File opening error\n");

    size_t ret_code = fread(buffer_ptr, sizeof(unsigned char), BUF_SIZE, fp);
...
void invshiftrows(unsigned char blockIn[4][4])//added on 19/10/15
{
    int temp;
    temp = blockIn[1][3];//row 2
    blockIn[1][3] = blockIn[1][2];
    blockIn[1][2] = blockIn[1][1];
    blockIn[1][1] = blockIn[1][0];
    blockIn[1][0] = temp;
    temp = blockIn[2][2];//row 3
    blockIn[2][2] = blockIn[2][0];
    blockIn[2][0] = temp;
    temp = blockIn[2][1];
    blockIn[2][1] = blockIn[2][3];
    blockIn[2][3] = temp;
    temp = blockIn[3][0];//row 3
    blockIn[3][0] = blockIn[3][1];
    blockIn[3][1] = blockIn[3][2];
    blockIn[3][2] = blockIn[3][3];
    blockIn[3][3] = temp;
}

void subByte(unsigned char* pIn)
{
    int i = (int)(*pIn) / 16;
    int j = (int)(*pIn) % 16;
...
void invSub(unsigned char* pIn)//added on 19/10/15
{
    int i = (int)(*pIn) / 16;
    int j = (int)(*pIn) % 16;
...
void invsubbytes(unsigned char blockIn[4][4])
{
    int i, j;
